  ...
  if (role !== "Admin") {
    return (
      <SectionCard title="Restricted">
        <p className="text-sm text-muted-foreground">Only Admins can view and manage administrators.</p>
      </SectionCard>
    );
  }
                ...
                <tr key={a.id} className="hover:bg-secondary/40">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <Avatar className="h-8 w-8"><AvatarFallback>{a.name[0]}</AvatarFallback></Avatar>
                      <div>
                  ...
                  <td className="px-4 py-3">
                    {a.status === "Active"
                      ...
                      : <Badge variant="outline" className="border-gold/40 text-gold-foreground bg-gold-soft">Pending</Badge>}
                  </td>
                  <td className="px-4 py-3 text-right">
                    <Button size="sm" variant="ghost" onClick={() => { removeAdmin(a.id); toast.success(`${a.name} removed`); }}>
...
function InviteAdminDialog({ invitedBy }: { invitedBy: string }) {
  ...
  function submit() {
    ...
    setOpen(false);
    setName(""); setEmail(""); setScope("Global");
  }

  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="bg-gradient-royal text-primary-foreground"><UserPlus className="mr-1 h-4 w-4" />Invite admin</Button>
      </DialogTrigger>
      <DialogContent className="max-w-md">
        <DialogHeader><DialogTitle>Invite another admin</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div className="space-y-1.5"><Label>Full name</Label><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Pst. Jane Doe" /></div>
          <div className="space-y-1.5"><Label>Email</Label><Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="jane@ministry.org" /></div>
          <div className="space-y-1.5"><Label>Scope</Label><Input value={scope} onChange={(e) => setScope(e.target.value)} placeholder="Global, region, or branch" /></div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button>
          <Button className="bg-gradient-royal text-primary-foreground" onClick={submit}>Send invite</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
